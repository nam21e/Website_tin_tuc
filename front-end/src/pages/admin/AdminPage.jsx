            ...
            <table className="min-w-full text-left text-sm">
              <thead className="text-xs text-slate-500">
                <tr>
                  ...
                  {onView ? <th className="px-3 py-2">Chi tiết</th> : null}
                  <th className="px-3 py-2"></th>
                </tr>
              </thead>
              <tbody>
                {(data || []).map((item) => (
                  <tr
                    key={item.id}
                    className={`border-t ${onView ? 'cursor-pointer hover:bg-slate-50' : ''}`}
                    onClick={() => onView?.(item)}
                  >
...
export function AdminPage() {
  ...
  return (
    <>
      ...
      <Modal
        open={Boolean(selectedUser) && tab === 'users'}
        title={selectedUser ? `Thông tin user #${selectedUser.id}` : 'Thông tin user'}
        ...
        {selectedUser ? (
          <div className="grid gap-3 text-sm">
            ...
            <div className="grid gap-1">
              ...
              <div className="text-slate-900">{selectedUser.role || '-'}</div>
            </div>
            <div className="grid gap-1">
              <div className="text-xs font-medium text-slate-500">Avatar</div>
